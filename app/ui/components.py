...
def render_calculator_input():
    ...
    with main_col_left:
        ...
        with c1:
            if curr_mode == "해지보고":
                ...
                equipment_type = st.selectbox("장비 구성", ["신품", "중품", "혼합"], help="투자되는 장비의 신규 여부를 선택하세요.")
                usage_period = st.radio("고객 사용기간", ["3년 미만", "3년 이상"], horizontal=True)
                val = 0
        with c2:
            if curr_mode != "해지보고":
                ...
                contract_period = st.selectbox("약정/재계약 기간", ["0", "12", "24", "36"], format_func=lambda x: {"0":"무약정", "12":"12개월", "24":"24개월", "36":"36개월"}[x])
            else:
                st.info("💡 30만원 이상 건은 결재 필수")
                delta_pct = 0; contract_period = "0"
        st.markdown('</div>', unsafe_allow_html=True)

        # Standard Emoji headers to avoid character breaking
        with st.expander("🛠️ 추가 비용 및 실무 가이드"):
    ...
    with main_col_right:
        res_inputs = {
            ...
            "equipment_type": "new" if equipment_type=="신품" else "used" if equipment_type=="중품" else "mix" if curr_mode in ["업셀링", "일반P.P"] else "replace",
            "usage_period": "under_3yr" if usage_period=="3년 미만" else "over_3yr" if curr_mode == "업셀링" else "under_3yr",
...
def render_results(inputs):
    ...
    if mode == "해지보고":
        ...
        pp_display = f"{pp:.2f}" if pp and pp > 0 else "0.00"
